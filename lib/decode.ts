...
function parseModelParts(model: string): { family: string; version: string } | null {
  const parts = model.split('-')
  const familyIdx = parts.findIndex(p => (MODEL_FAMILIES as readonly string[]).includes(p))
  if (familyIdx === -1) return null
  const family = parts[familyIdx]
  const versionNums: string[] = []
  for (const p of parts.slice(familyIdx + 1)) {
    if (/^\d{1,2}$/.test(p)) versionNums.push(p)
    else break
  }
  return { family, version: versionNums.join('.') }
}

/** Human-readable: "claude-opus-4-7-20250514" → "Opus 4.7" */
export function modelDisplayName(model: string): string {
  const parsed = parseModelParts(model)
  if (!parsed) return model
  const cap = parsed.family.charAt(0).toUpperCase() + parsed.family.slice(1)
  return parsed.version ? `${cap} ${parsed.version}` : cap
}

/** Compact technical ID: "claude-opus-4-7-20250514" → "claude-opus-4.7" */
export function modelShortId(model: string): string {
  const parsed = parseModelParts(model)
  if (!parsed) return model
  return parsed.version ? `claude-${parsed.family}-${parsed.version}` : `claude-${parsed.family}`
}
